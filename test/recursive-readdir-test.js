...
describe("readdir", function () {
  it("correctly lists all files in nested directories", function (done) {
    ...
    readdir(p.join(__dirname, "testdir"), false, function (err, list) {
      assert.ifError(err);
      assert.deepEqual(list.map((l) => l.path).sort(), expectedFiles.sort());
      done();
    });
  });
    ...
    readdir(
      ...
      function (err, list) {
        ...
        list.forEach(function (file) {
          assert.equal(
            notExpectedFiles.indexOf(file.path),
            -1,
            'Failed to ignore file "' + file.path + '".'
          );
        });
        done();
      }
  ...
  it("supports ignoring files with the globstar syntax", function (done) {
    ...
    readdir(p.join(__dirname, "testdir"), false, ignores, function (err, list) {
      assert.ifError(err);
      list.forEach(function (file) {
        assert.equal(
          notExpectedFiles.indexOf(file.path),
    ...
    it("ignores files that the function returns true for", function (done) {
      ...
      readdir(
        ...
        function (err, list) {
          ...
          list.forEach(function (file) {
            assert.equal(
              ignoredFiles.indexOf(file.path),
              -1,
              'Failed to ignore file "' + file.path + '".'
            );
          });
  ...
  if (!global.Promise) {
    ...
    it("works with promises", function (done) {
      ...
      readdir(p.join(__dirname, "testdir"), false)
        .then(function (list) {
          assert.deepEqual(
            list.map((l) => l.path).sort(),
            expectedFiles.sort()
    ...
    it("correctly ignores when using promises", function (done) {
      ...
      readdir(p.join(__dirname, "testdir"), false, ["*.txt"])
        .then(function (list) {
          assert.deepEqual(
            list.map((l) => l.path).sort(),
            expectedFiles.sort()
    ...
    it("correctly fetch file stats when in outputFileStats mode", function (done) {
      readdir(p.join(__dirname, "testdir"), true)
        .then((res) => {
          assert(
            res.every((obj) => "size" in obj),
            'All objects must have "size" key'
          );
          done();
        })
        .catch(done);
